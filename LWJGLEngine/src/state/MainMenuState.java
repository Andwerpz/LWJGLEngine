...
public class MainMenuState extends State {
	...
	if (perspectiveScreen == null) {
	    perspectiveCamera = new Camera(PERSPECTIVE_FOV, (float) Main.windowWidth, (float) Main.windowHeight,
		    Main.NEAR, Main.FAR);
	...
	if (uiScreen == null) {
	    uiScreenCamera = new Camera(
		    Mat4.orthographic(0, (float) Main.windowWidth, 0, (float) Main.windowHeight, -10, 10));
	...
	Model.addInstance(AssetManager.getModel("dust2"),
	...
	InputManager.addInput("btn_host_game", new Button(150, Main.windowHeight / 2 - 0, 200, 30, "Host Game",
		FontUtils.CSGOFont, 32, Scene.UI_SCENE));
	InputManager.addInput("btn_join_game", new Button(150, Main.windowHeight / 2 - 40, 200, 30, "Join Game",
		FontUtils.CSGOFont, 32, Scene.UI_SCENE));
	InputManager.addInput("btn_settings", new Button(150, Main.windowHeight / 2 - 80, 200, 30, "Settings",
		FontUtils.CSGOFont, 32, Scene.UI_SCENE));
	InputManager.addInput("btn_quit_game", new Button(150, Main.windowHeight / 2 - 120, 200, 30, "Quit Game",
		FontUtils.CSGOFont, 32, Scene.UI_SCENE));
	...
	InputManager.addInput("tf_host_port", new TextField(370, Main.windowHeight / 2 - 0, 180, 30, "Port",
		new Font("Dialogue", Font.PLAIN, 1), 16, Scene.UI_SCENE));

	InputManager.addInput("tf_join_ip", new TextField(370, Main.windowHeight / 2 - 40, 180, 30, "IP",
		new Font("Dialogue", Font.PLAIN, 1), 16, Scene.UI_SCENE));
	InputManager.addInput("tf_join_port", new TextField(570, Main.windowHeight / 2 - 40, 180, 30, "Port",
		new Font("Dialogue", Font.PLAIN, 1), 16, Scene.UI_SCENE));
    }
	...
	perspectiveCamera.setFacing(0, MathUtils.interpolate(0, 0, (float) Math.PI * 2f, ROTATION_TIME * 1000f,
